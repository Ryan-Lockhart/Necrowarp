...
namespace necrowarp {
	template<map_type_e MapType> inline command_pack_t mercenary_t::think() const noexcept {
	...
	template<map_type_e MapType> inline void mercenary_t::die() noexcept {
		object_registry<MapType>.template add<true>(skull_t{ position });
		object_registry<MapType>.template add<true>(metal_t{ position });

		fluid_map<MapType>[position] += fluid_type<mercenary_t>::type;
