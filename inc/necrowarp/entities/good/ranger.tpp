...
namespace necrowarp {
	template<map_type_e MapType> inline command_pack_t ranger_t::think(offset_t position) const noexcept {
		...
		} else if (entity_registry<MapType>.dependent nearby<distance_function_e::Chebyshev, ALL_NON_GOOD>(position) || (!can_nock() && !can_loose() && object_registry<MapType>.dependent empty<arrow_t>())) {
			for (cauto offset : neighbourhood_offsets<distance_function_e::Chebyshev>) {
				const offset_t current_position{ position + offset };
