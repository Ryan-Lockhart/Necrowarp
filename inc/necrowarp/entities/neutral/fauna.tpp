...
namespace necrowarp {
	...
	template<map_type_e MapType> inline command_pack_t fauna_t::think(offset_t position) const noexcept {
		...
		const bool threat_nearby{
			entity_registry<MapType>.dependent nearby<distance_function_e::Chebyshev, ALL_NON_NEUTRAL>(position) || entity_registry<MapType>.dependent nearby<distance_function_e::Chebyshev, fauna_t>(position)
		};
		...
		for (cauto offset : neighbourhood_offsets<distance_function_e::Chebyshev>) {
			...
			if (object_registry<MapType>.dependent contains<flora_t>(current_position) && !threat_nearby && is_hungry) {
