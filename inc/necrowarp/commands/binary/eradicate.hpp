...
namespace necrowarp {
	...
	template<> struct is_command<eradicate_t> {
		static constexpr bool value = true;
	};

	template<> struct is_binary_command<eradicate_t> {
		static constexpr bool value = true;
	};

	template<> struct to_command_type<command_e::Eradicate> {
		using type = eradicate_t;
	};

	template<> struct to_command_enum<eradicate_t> {
