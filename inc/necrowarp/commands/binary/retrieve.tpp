...
namespace necrowarp {
	template<NonNullEntity EntityType> template<map_type_e MapType> inline void entity_command_t<EntityType, retrieve_t>::process() const noexcept {
		...
		ptr<arrow_t> maybe_arrow{ object_registry<MapType>.template at<arrow_t>(target_position) };

		if (maybe_arrow == nullptr || !object_registry<MapType>.template remove<arrow_t>(target_position)) {
			return;
		}
