...
namespace necrowarp {
	...
	template<> struct is_command<calcitic_invocation_t> {
		static constexpr bool value = true;
	};

	template<> struct is_binary_command<calcitic_invocation_t> {
		static constexpr bool value = true;
	};

	template<> struct is_grimoire<calcitic_invocation_t> {
		static constexpr bool value = true;
	};

	template<> struct to_grimoire_enum<calcitic_invocation_t> {
		static constexpr grimoire_e value = grimoire_e::CalciticInvocation;
	};
	...
	template<NonNullEntity EntityType> struct entity_command_t<EntityType, calcitic_invocation_t> {
		using entity_type = EntityType;
		using command_type = calcitic_invocation_t;
