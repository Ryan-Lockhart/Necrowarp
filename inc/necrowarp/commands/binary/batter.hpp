...
namespace necrowarp {
	...
	template<> struct is_command<batter_t> {
		static constexpr bool value = true;
	};

	template<> struct is_binary_command<batter_t> {
		static constexpr bool value = true;
	};

	template<> struct to_command_type<command_e::Batter> {
		using type = batter_t;
	};

	template<> struct to_command_enum<batter_t> {
